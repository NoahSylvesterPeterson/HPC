...
void Jacobi(VD &Solution, mpiInfo &myMPI)
{
  ...
  while (global_converged == 0 && ++iter <= max_iter)
  {
    ...
    rLOOP
    {
      ...
      for (int c = 2; c <= bandwidth; ++c)
        RowSum_sumPE[r] -= Acoef[r][c] * Solution[Jcoef[r][c]];
    }
  ...
  if (global_converged == 1)
    if (myMPI.myPE == 0)  cout << "Jacobi converged in " << iter << " iterations.\n" ;
      if (global_converged == 0)
        if (myMPI.myPE == 0)  cout << "Jacobi failed to converge aftger " << iter << " iterations.\n" ;
}
...
void CG(VD &Solution, mpiInfo &myMPI)
{
  ...
  if (global_converged == 1)
    if (myMPI.myPE == 0) cout << "CG converged in " << iter << " iterations.\n" ;
      if (global_converged == 0)
        if (myMPI.myPE == 0)  cout << "CG failed to converge aftger " << iter << " iterations.\n" ;
}
