  ...
  rowLOOP
    {
      ...
      ANNOTATE_ITERATION_TASK(MatrixProductLoops);
      colLOOP
	    {
  ...
  while ( global_converged == 0  && ++iter <= max_iter)
    {
      ...
      ANNOTATE_SITE_BEGIN(cg search direction);
      ANNOTATE_ITERATION_TASK(update p);
      rowLOOP p[row] = rnew[row] + beta*p[row];
      ANNOTATE_SITE_END();
      
      // (4.5) r "new" will be r "old" for next iteration
      ANNOTATE_SITE_BEGIN(cg update r);
      ANNOTATE_ITERATION_TASK(update r);
